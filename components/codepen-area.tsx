...
const CodePenArea = (props: any) => {
  ...
  const getCodepenData = useCallback(async () => {
    ...
    const { items = [] } = data
    const sortedData: codepenItemType[] = items.sort((a: { pubDate: string; }, b: { pubDate: string; }) => {
  ...
  return <div style={{ display: 'flex' }}>
    {
      popularPen.map((item, idx) => {
        ...
        return <iframe height="300" key={'ifram-' + idx}
          scrolling="no" title={item?.title} src={previewUrl}
